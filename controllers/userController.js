...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    await Promise.all([
      User.findByIdAndUpdate(targetId, { $addToSet: { "friendRequests.received": requesterId } }),
      User.findByIdAndUpdate(requesterId, { $addToSet: { "friendRequests.sent": targetId } })
    ]);
...
export const acceptFriendRequest = async (req, res) => {
  try {
    ...
    const updatedUser = await User.findById(userId).populate("friends", "name username profilePicture");
...
export const declineFriendRequest = async (req, res) => {
  try {
    ...
    await Promise.all([
      User.findByIdAndUpdate(userId, { $pull: { "friendRequests.received": requesterId } }),
      User.findByIdAndUpdate(requesterId, { $pull: { "friendRequests.sent": userId } })
    ]);
